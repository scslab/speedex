...
void 
SerialAccountModificationLog::log_self_modification(
	AccountID owner, uint64_t sequence_number) {
	modification_log.template insert<LogInsertFn, uint64_t>(
		owner, sequence_number);

}

void 
SerialAccountModificationLog::log_other_modification(
	AccountID tx_owner, uint64_t sequence_number, AccountID modified_account) {
...
AccountModificationLog::diff_with_prev_log(uint64_t block_number) {
	...
	for (size_t i = 0; i < std::min(prev.size(), current.size()); i++) {
		...
		if (p.new_transactions_self.size() != c.new_transactions_self.size()) {
			...
			if (found_dif) {
				for (unsigned int j = 0; j < p.new_transactions_self.size(); j++) {
					std::printf("new_transactions_self prev_seqnum %" PRIu64 " current_seqnum %" PRIu64 "\n", 
					...
					std::printf("new_transactions_self prev_owner %" PRIu64 " current_owner %" PRIu64 "\n", 
		...
		if (p.identifiers_self.size() != c.identifiers_self.size()) {
			...
			if (found_dif) {
			std::printf("%lu (account prev %" PRIu64 " / current %" PRIu64 ") different identifiers_self\n", i, p.owner, c.owner);
			for (size_t j = 0; j < p.identifiers_self.size(); j++) {
				//if (p.identifiers_self[j] != c.identifiers_self[j]) {
					std::printf("%lu %lu identifiers_self_prev_seqnum %" PRIu64 " current_seqnum %" PRIu64 "\n", i, j, p.identifiers_self[j], c.identifiers_self[j]);
				//}
			}
			}
		}
