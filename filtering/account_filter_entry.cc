...
AccountFilterEntry::add_req(AssetID const& asset, int64_t amount)
{
	...
	if (__builtin_add_overflow_p(amount, required_assets[asset], static_cast<int64_t>(0)))
	{
...
AccountFilterEntry::compute_reqs()
{
	for (auto const& [_, tx] : txs)
	{
		for (auto const& op : tx.transaction.operations)
		{
			...
			{
				case CREATE_ACCOUNT:
					add_req(MemoryDatabase::NATIVE_ASSET, op.body.createAccountOp().startingBalance);
					break;
				case CREATE_SELL_OFFER:
					add_req(op.body.createSellOfferOp().category.sellAsset, op.body.createSellOfferOp().amount);
					break;
				case CANCEL_SELL_OFFER:
					break;
				case PAYMENT:
					add_req(op.body.paymentOp().asset, op.body.paymentOp().amount);
					break;
				case MONEY_PRINTER:
					break;
				default:
...
void 
AccountFilterEntry::add_tx(SignedTransaction const& tx, MemoryDatabase const& db)
{
	if (found_error) {
		return;
	}
